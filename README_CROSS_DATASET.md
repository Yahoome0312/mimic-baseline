# 跨数据集测试功能总结

## ✅ 功能已实现

现在项目支持：
1. **训练**: MIMIC-CXR 数据集 (377,095 张图像)
2. **测试**: ChestXray14 数据集 (25,596 张测试图像)
3. **Zero-shot 推理**: 无需标签映射，自动适配

---

## 📁 新增文件

### 1. 数据加载器
- **`datasets/chestxray14_dataset.py`**
  - ChestXray14Dataset 类
  - ChestXray14DataLoader 类
  - 支持 14 个病理类别
  - 自动处理多标签数据

### 2. 测试脚本
- **`test_chestxray14_data.py`**
  - 验证 ChestXray14 数据加载
  - 显示数据集统计信息
  - 检查图像路径有效性

### 3. 批处理脚本
- **`run_cross_dataset_test.bat`**
  - 一键运行跨数据集训练和测试
  - 自动化工作流程

### 4. 文档
- **`CROSS_DATASET_GUIDE.md`**
  - 完整使用指南
  - 实验设计建议
  - 故障排查方案

- **`README_CROSS_DATASET.md`** (本文件)
  - 功能总结
  - 快速开始指南

---

## 🚀 快速开始

### Step 1: 测试 ChestXray14 数据加载

```bash
python test_chestxray14_data.py
```

**预期输出**:
```
[OK] Data loaded successfully!
  Total test images: 25596
  Label shape: (25596, 14)
```

### Step 2: 跨数据集训练和测试

**选项 A - 使用批处理脚本**:
```bash
run_cross_dataset_test.bat
```

**选项 B - 命令行**:
```bash
# 在 MIMIC 训练，在 ChestXray14 测试
python main.py --method finetune --test_chestxray14
```

**选项 C - Zero-shot（无需训练）**:
```bash
# 直接在 ChestXray14 进行 zero-shot 测试
python main.py --method zeroshot --test_chestxray14
```

---

## 📊 数据集对比

| 特性 | MIMIC-CXR | ChestXray14 |
|------|-----------|-------------|
| **用途** | 训练 + 验证 | 测试 |
| **图像数** | 377,095 | 25,596 (测试集) |
| **类别数** | 14 | 14 |
| **来源** | 贝斯以色列医院 | NIH |
| **标注质量** | 高（放射科报告） | 中（NLP提取） |

---

## 🎯 主要改动

### 1. 新增命令行参数

```bash
--test_chestxray14          # 使用外部测试集
--test_chestxray14_path     # 指定外部数据集路径（可选）
```

### 2. 自动文件命名

**MIMIC 测试集**:
- `zeroshot_results.json`
- `finetune_results.json`

**ChestXray14 测试集**:
- `zeroshot_on_ChestXray14_results.json`
- `finetune_on_ChestXray14_results.json`

### 3. Zero-shot 文本提示

自动为 ChestXray14 类别生成合适的文本提示：
```
"chest x-ray showing atelectasis"
"chest x-ray showing cardiomegaly"
"chest x-ray showing infiltration"
...
```

---

## 💡 使用场景

### 场景 1: 基线性能评估
```bash
# 在 MIMIC 上训练和测试（同分布）
python main.py --method finetune
```

### 场景 2: 泛化能力测试
```bash
# 在 MIMIC 训练，在 ChestXray14 测试（跨分布）
python main.py --method finetune --test_chestxray14
```

### 场景 3: Zero-shot 能力评估
```bash
# 不训练，直接 zero-shot 测试 ChestXray14
python main.py --method zeroshot --test_chestxray14
```

### 场景 4: 损失函数对比
```bash
# 标准损失
python main.py --method finetune --test_chestxray14 --loss_type standard

# 加权损失（处理类别不平衡）
python main.py --method finetune --test_chestxray14 --loss_type weighted

# Focal 损失（关注难例）
python main.py --method finetune --test_chestxray14 --loss_type focal
```

---

## 📈 预期性能

根据文献和经验：

### MIMIC 测试集（同分布）
- **Subset Accuracy**: 25-35%
- **Hamming Loss**: 0.05-0.10
- **F1-score (Macro)**: 0.60-0.75
- **AUC (Macro)**: 0.75-0.85

### ChestXray14 测试集（跨分布）
- **Subset Accuracy**: 15-25%（下降 ~10%）
- **Hamming Loss**: 0.08-0.15（上升）
- **F1-score (Macro)**: 0.40-0.60（下降 ~15-20%）
- **AUC (Macro)**: 0.65-0.75（下降 ~10%）

**性能下降原因**:
1. 数据分布差异（医院、设备、患者群体）
2. 标注方式不同（MIMIC 用放射科报告，ChestXray14 用 NLP）
3. 类别定义略有差异

---

## 🔍 技术细节

### 为什么不需要标签映射？

传统的跨数据集方法需要手动映射类别，例如：
```python
# 传统方法需要这样
label_mapping = {
    'Pleural Effusion': 'Effusion',
    ...
}
```

**我们的方法使用 Zero-shot**:
1. 为每个类别生成文本提示
2. 用 CLIP 计算图像-文本相似度
3. 自动适配不同类别名称

```python
# MIMIC 类别: "Pleural Effusion"
text_prompt = "chest x-ray showing pleural effusion"

# ChestXray14 类别: "Effusion"
text_prompt = "chest x-ray showing effusion"

# CLIP 自动理解两者语义相同！
```

### 数据流程图

```
┌─────────────────────┐
│  MIMIC-CXR 数据集   │
│  (训练 + 验证)      │
└──────────┬──────────┘
           │
           ↓
   ┌───────────────┐
   │ BiomedCLIP   │
   │  训练/微调    │
   └───────┬───────┘
           │
           ↓
    保存最佳模型
           │
           ↓
  ┌────────┴────────┐
  │                 │
  ↓                 ↓
MIMIC 测试      ChestXray14
(同分布)        (跨分布)
  │                 │
  ↓                 ↓
评估指标         评估指标
```

---

## 🛠️ 自定义配置

### 指定 ChestXray14 路径

如果数据不在默认位置：
```bash
python main.py --test_chestxray14 \
  --test_chestxray14_path "E:\MyData\ChestXray14"
```

### 调整训练参数

```bash
python main.py --method finetune --test_chestxray14 \
  --batch_size 16 \
  --epochs 30 \
  --lr 5e-6
```

### 实验命名

```bash
python main.py --method finetune --test_chestxray14 \
  --experiment_name "focal_loss_exp1" \
  --loss_type focal \
  --focal_gamma 2.5
```

---

## 📋 ChestXray14 类别列表

完整的 14 个类别：

1. **Atelectasis** (肺不张) - 12.81%
2. **Cardiomegaly** (心脏肥大) - 4.18%
3. **Consolidation** (肺实变) - 7.09%
4. **Edema** (水肿) - 3.61%
5. **Effusion** (积液) - 18.20% ⬆️
6. **Emphysema** (肺气肿) - 4.27%
7. **Fibrosis** (纤维化) - 1.70%
8. **Hernia** (疝气) - 0.34% ⬇️ 最少
9. **Infiltration** (浸润) - 23.88% ⬆️ 最多
10. **Mass** (肿块) - 6.83%
11. **Nodule** (结节) - 6.34%
12. **Pleural_Thickening** (胸膜增厚) - 4.47%
13. **Pneumonia** (肺炎) - 2.17%
14. **Pneumothorax** (气胸) - 10.41%

---

## ⚠️ 常见问题

### Q1: 为什么跨数据集性能会下降？

**A**: 这是正常现象，原因：
- 数据分布差异（Domain Shift）
- 标注方式不同
- 图像采集设备和协议差异

**性能下降幅度反映了模型的泛化能力**。

### Q2: 如何提高跨数据集性能？

**A**: 可以尝试：
1. 使用 Domain Adaptation 技术
2. 增加训练数据多样性
3. 使用更强的数据增强
4. 尝试不同的损失函数（weighted/focal）

### Q3: 能否在 ChestXray14 上训练？

**A**: 可以，但需要：
1. 修改 `config.py` 中的路径和类别
2. 使用 `chestxray14_dataset.py` 替代 `mimic_dataset.py`
3. ChestXray14 没有官方的 train/val 分割，需要自己划分

### Q4: 两个数据集能否混合训练？

**A**: 可以，但需要：
1. 统一类别定义
2. 创建混合数据加载器
3. 注意标注质量差异

---

## 📚 参考文献

1. **MIMIC-CXR**: Johnson et al., "MIMIC-CXR Database", *Scientific Data*, 2019
2. **ChestXray14**: Wang et al., "ChestX-ray8", *CVPR*, 2017
3. **BiomedCLIP**: Zhang et al., "BiomedCLIP", 2023

---

## 🎓 学术用途

### 实验设计示例

**研究问题**: BiomedCLIP 在跨医院胸部X光数据集上的泛化能力

**实验设置**:
1. **训练**: MIMIC-CXR (波士顿贝斯以色列医院)
2. **测试**: ChestXray14 (NIH 多医院汇总)
3. **对比**: 同分布 vs 跨分布性能

**评估指标**:
- AUC-ROC (per-class and macro)
- F1-score (per-class and macro)
- Subset Accuracy
- Hamming Loss

**预期贡献**:
- 量化模型泛化能力
- 识别对分布变化敏感的类别
- 为临床部署提供参考

---

## ✅ 验证清单

在运行跨数据集测试前，请确认：

- [ ] ChestXray14 数据已下载到 `D:\Data\ChestXray14\CXR8`
- [ ] 运行 `test_chestxray14_data.py` 成功
- [ ] 至少有 32GB 内存（或调小 batch_size）
- [ ] CUDA GPU 可用（推荐）
- [ ] 有足够磁盘空间保存结果（~1GB）

---

**创建日期**: 2025-12-15
**作者**: Claude
**版本**: 1.0
**状态**: ✅ 功能完整，测试通过

详细技术文档请参考 `CROSS_DATASET_GUIDE.md`
